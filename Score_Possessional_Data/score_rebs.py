...
def score_rebs(player_events_df, game_df, current_player, home_away, team_id, offense_possession_value, defense_possession_value):
    ...
    oreb = pd.DataFrame(columns=['EVENTNUM', 'EVENTMSGTYPE', 'WINNINGTEAM', 'NEUTRALDESCRIPTION',
       'LOSINGTEAM', 'PLAYER1_ID', 'PLAYER1_NAME', 'PLAYER2_ID',
       'PLAYER2_NAME', 'PLAYER3_ID', 'PLAYER3_NAME'])
    dreb = pd.DataFrame(columns=['EVENTNUM', 'EVENTMSGTYPE', 'WINNINGTEAM', 'NEUTRALDESCRIPTION',
       'LOSINGTEAM', 'PLAYER1_ID', 'PLAYER1_NAME', 'PLAYER2_ID',
       'PLAYER2_NAME', 'PLAYER3_ID', 'PLAYER3_NAME'])

    # Populate dataframes with appropriate rebound types
    for idx, event in player_events_df['EVENTMSGTYPE'].iteritems():
        if 'miss' in str(game_df.loc[idx - 1]['WINNINGTEAM']).lower():
            oreb = oreb.append(player_events_df.loc[idx][:])
        elif 'miss' in str(game_df.loc[idx - 1]['LOSINGTEAM']).lower():
    ...
    for idx, event in oreb['EVENTMSGTYPE'].iteritems(): # For every o-reb
        ...
        while True:
            ...
            or ('miss' in str(game_df.loc[idx + shot_made_index]['WINNINGTEAM']).lower() and game_df.loc[idx + shot_made_index + 1]['EVENTMSGTYPE'] == 4 and game_df.loc[idx + shot_made_index + 1]['LOSINGTEAM'] != None)): # Missed shot rebounded by the losing team
            ...
            and game_df.loc[idx + shot_made_index]['PLAYER1_TEAM_ID'] == team_id): # Winning Team Check
                if '3pt' not in str(game_df.loc[idx + shot_made_index]['WINNINGTEAM']).lower(): 
            ...
            and game_df.loc[idx + shot_made_index]['PLAYER1_TEAM_ID'] == team_id): # Winning Team Check
                counter = 0
                if 'of 1' in str(game_df.loc[idx + shot_made_index]['WINNINGTEAM']).lower(): # Amount of FT check
                    free_throws = 1
                elif 'of 2' in str(game_df.loc[idx + shot_made_index]['WINNINGTEAM']).lower(): # Amount of FT check
                    free_throws = 2
                elif 'of 3' in str(game_df.loc[idx + shot_made_index]['WINNINGTEAM']).lower(): # Amount of FT check
                    free_throws = 3
                else:
                    print('O-REBS: ', str(game_df.loc[idx]['WINNINGTEAM']).lower())
                    print('O-REBS: ', str(game_df.loc[idx + shot_made_index]['WINNINGTEAM']).lower())
                    print('O-REBS: ERROR: UNKNOWN NUMBER OF FREE THROWS', idx)
                    break
                
                while free_throws > 0:
                    if 'miss' not in str(game_df.loc[idx + shot_made_index + counter]['WINNINGTEAM']).lower():
                        ...
                        counter += 1
                break
            elif ('miss' in str(game_df.loc[idx + shot_made_index]['WINNINGTEAM']).lower() # Missed Shot by Winning Team
            and game_df.loc[idx + shot_made_index + 1]['EVENTMSGTYPE'] == 4 # Rebound Following Missed Shot
            and game_df.loc[idx + shot_made_index + 1]['WINNINGTEAM'] != None): # Winning Team Offensive Rebound
        ...
        if shot_not_made == 1:
            ...
            while True:
                ...
                or (game_df.loc[idx - o_board_index]['EVENTMSGTYPE'] == 1 and game_df.loc[idx - o_board_index]['WINNINGTEAM'] == None)): 
                    break
                elif ('miss' in str(game_df.loc[idx - o_board_index - 1]['LOSINGTEAM']).lower()
                ...
                and game_df.loc[idx - o_board_index]['WINNINGTEAM'] == None): # If the losing team takes a free throw break the loop
                    break
                elif ('miss' in str(game_df.loc[idx - o_board_index - 1]['WINNINGTEAM']).lower()
                ...
                and 'miss' in str(game_df.loc[idx - o_board_index - 1]['WINNINGTEAM']).lower()
            ...
            if points_on_board < offense_possession_value: #Don't penalize offensive rebounder if free throw shooter missed shots
                continue
            elif 'ast' in str(game_df.loc[idx + shot_made_index]['WINNINGTEAM']).lower(): # If basket was assisted
    ...
    for idx, event in dreb['EVENTMSGTYPE'].iteritems(): # For every d-reb
        ...
        while True:
            ...
            elif 'block' in str(game_df.loc[idx - counter]['WINNINGTEAM']).lower(): # If there was a block before a change of possession

...
def score_blocks(game_df, general_df, current_player, win_loss, team_id, defense_possession_value):
    ...
    player_events_df = game_df[(game_df[team1].str.contains("BLOCK", na=False)) & (game_df[team1].str.contains(player_last_name, na=False))]
        ...
        while True:
            if idx + shot_made_index > len(game_df.index) - 1: # If we go to an index outside the game_df, there must be no score after the block
                ...
                break
                  ...
                  and game_df.loc[idx + shot_made_index][team1] == None): # If we see that the loing team made a shot assign no value to the block and move onto the next block to be evaluated
                break

            elif (game_df.loc[idx + shot_made_index]['EVENTMSGTYPE'] == 4
                  and game_df.loc[idx + shot_made_index][team1] != None): # If there is a rebound by the winning team, award value to the block and move onto next block
                ...
                break
                 ...
                 and 'steal' in str(game_df.loc[idx + shot_made_index][team1]).lower()): # If a steal caused a change in possession by the losing team, award the block value based on value structure with turnover
                current_player_blocks_score += defense_possession_value * block_before_turnover
                break

            elif (game_df.loc[idx + shot_made_index]['EVENTMSGTYPE'] == 5 
                 and game_df.loc[idx + shot_made_index][team1] == None): # If there was a turnover that was not a steal, award 100% value for block
                ...
                break
                  ...
                  and game_df.loc[idx + shot_made_index + 1][team2] == None)): # If there was a missed shot followed by a rebound by the winning team
                ...
                if 'technial' in str(game_df.loc[idx + shot_made_index][team2]).lower(): # If free throws caused by technical foul
                    free_throws = 1
                    if 'pts' in str(game_df.loc[idx + shot_made_index + counter][team2]).lower(): # If tehnical free throw is made
                        ...
                        free_throws -= 1
                        counter += 1
                elif 'of 1' in str(game_df.loc[idx + shot_made_index][team2]).lower(): # If there was 1 free throw
                    free_throws = 1
                elif 'of 2' in str(game_df.loc[idx + shot_made_index][team2]).lower(): # If there were 2 free throws
                    free_throws = 2
                elif 'of 3' in str(game_df.loc[idx + shot_made_index][team2]).lower(): # If there were 3 free throws
                    free_throws = 3
                else: # Print errors if there are an unknown number of free throws
                    print('BLOCK: ', str(game_df.loc[idx][team2]).lower())
                    print('BLOCK: ', str(game_df.loc[idx + shot_made_index][team2]).lower())
                ...
                while free_throws > 0: # Continue going down the game_df if there are still free throw shots to be taken
                    if game_df.loc[idx + shot_made_index + counter]['EVENTMSGTYPE'] == 4: # There is a nuance that team rebounds are often included in between free throws. If there is a rebound in the free throw sequence, move to th next index
                        counter += 1
                        continue
                    elif 'miss' not in str(game_df.loc[idx + shot_made_index + counter][team2]).lower(): # If a free throw is made
                ...
                if (idx + shot_made_index + counter + 1 > len(game_df.index) - 1
                    ...
                    and game_df.loc[idx + shot_made_index + counter][team1] != None): # If there is a defensive rebound after the free throw, give credit to the block based on the amount of free throws made

...
def one_game_wins(game_id, game_df, general_df, current_team_id, home_away, winning_team, losing_team):
    
    # Create dataframe for value box score
    df = pd.DataFrame(columns=['game_id', 'team_id', 'player_id','player_name','points_score','assists_score','orebs_score', 'drebs_score', 'to_score', 'stls_score','blocks_score','ft_score','dfg_score','sast_score', 'ft_ast_score', 'missed_fg_score', 'def_fouls_score'])
    # Get possession variables
    offense_possession_value, defense_possession_value, effective_ft_pct = possession_variables(game_id, winning_team, losing_team)

    # Loop through every player on the winning team
    for idx, player_id in general_df['PLAYER_ID'].iteritems():
        ...
        if general_df.loc[idx]['FG2M'] != 0 or general_df.loc[idx]['FG3M'] != 0:
            points_score = score_points(player_events_df, game_df, player_id, home_away, current_team_id, offense_possession_value) * general_df.loc[idx]['offense_factor2']
        else:
            points_score = 0

        # Score assists for each player for current game
        if general_df.loc[idx]['AST'] != 0:
            assists_score = score_assists(player_events_df, game_df, player_id, home_away, current_team_id, offense_possession_value) * general_df.loc[idx]['offense_factor2']
        else:
            assists_score = 0
        
        # Score rebounds for each player for current game
        if general_df.loc[idx]['OREB'] != 0 or general_df.loc[idx]['DREB'] != 0:
            rebs_score = score_rebs(player_events_df, game_df, player_id, home_away, current_team_id, offense_possession_value, defense_possession_value)
        ...
        if general_df.loc[idx]['TO'] != 0:
            to_score = score_to(player_events_df, game_df, player_id, home_away, current_team_id, offense_possession_value) * general_df.loc[idx]['offense_factor']
        else:
            to_score = 0

        # Score blocks for each player for current game
        if general_df.loc[idx]['BLK'] != 0:
            blocks_score = score_blocks(game_df, general_df, player_id, home_away, current_team_id, defense_possession_value) * general_df.loc[idx]['defense_factor2']
        else:
            blocks_score = 0

        # Score FT for each player for current game
        if general_df.loc[idx]['FTA'] != 0:
            ft_score = score_ft(player_events_df, game_df, player_id, home_away, current_team_id, offense_possession_value) * general_df.loc[idx]['offense_factor2']
        else:
            ft_score = 0
        ...
        if general_df.loc[idx]['STL'] != 0:
            stls_score = score_stls(player_events_df, game_df, player_id, home_away, current_team_id, defense_possession_value) * general_df.loc[idx]['defense_factor2']
        else:
            stls_score = 0

        # Score missed field goals
        missed_fg_score = score_missed_fg (player_events_df, game_df, player_id, offense_possession_value, winning_team) * general_df.loc[idx]['offense_factor']
        
        # Score defensive fouls
        def_fouls_score = score_def_foul(player_events_df, game_df, player_id, defense_possession_value) * general_df.loc[idx]['defense_factor']
        ...
        df.loc[idx] = [game_id, winning_team, player_id, player_name, points_score, assists_score, rebs_score[0], rebs_score[1], to_score, stls_score, blocks_score, ft_score, dfg_score, sast_score, ft_ast_score, missed_fg_score, def_fouls_score]
    ...
    df['team_id'] = winning_team
    df['opponent_id'] = losing_team

    
    return df[['game_id', 'team_id', 'opponent_id', 'player_id','player_name','points_score','assists_score','orebs_score', 'drebs_score', 'to_score', 'stls_score', 'blocks_score','ft_score','dfg_score','sast_score', 'ft_ast_score', 'missed_fg_score', 'def_fouls_score', 'jacob_value', 'wins_contr']]
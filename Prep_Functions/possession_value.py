...
def possession_value(game_id, winning_team, losing_team, current_team):
    ...
    loser_offense_possession_value = (loser_fg_points + losing_team_df.loc[0]['FTM']) / (loser_fg_att + losing_team_df.loc[0]['TO'] + (losing_team_df.loc[0]['FTA'] / 2)  - 1) # Minus 1 on attempts to subtract the missed shot in question, add in turnovers and free throws because you are effectively preventing another possession   
    
    if current_team == 1:
        return winner_offense_possession_value, loser_offense_possession_value, winner_effective_ft_pct
    else:
        return loser_offense_possession_value, winner_offense_possession_value, loser_effective_ft_pct 